...
after(async()=>{const {db}=await import('../db.js');db.close();await fs.rm(root,{recursive:true,force:true});});

function fixture({feet=false,fail=false,edit=false,cancel=false}={}) {
  ...
  const dependencies={getEntity:(entity)=>entity==='EpisodeVisualReview'?entry:record,
    upsertEntity:(...args)=>writes.push(args),listEntitiesByExactCriteria:()=>[],
  ...
  return {run:()=>createEpisodeReviewHandler(invoke,dependencies)({entity:'Session',recordId:'r',episodeId:'e',signature:episodeSignature(e)},{jobId:'j',signal:controller.signal,updateProgress(){}}),workspaces,writes,calls};
}
test('full episode, source offset, independent pass then prior comparison, one durable result',async()=>{
  const f=fixture();await f.run();
  const result=f.writes[0][2].result;
  ...
  assert.equal(f.calls.length,5);assert.match(f.calls.at(-1).prompt,/Earlier episodes/);assert.equal(f.writes.length,1);
...
test('feet retains its specialized lane and temporal evidence',async()=>{
  const f=fixture({feet:true});await f.run();const result=f.writes[0][2].result;
...
test('automatic completion, deduplication, missing fill and re-run keep episode identity',async()=>{
  ...
  registerJobHandler('episode_visual_review',async()=>({synthetic:true}));
