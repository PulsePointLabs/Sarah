...
test('vitals questions retrieve measured session-window glucose and SpO2 evidence', () => {
  ...
  assert.match(evidence.map((item) => item.text).join(' '), /Blood glucose: 1 session-window reading, average 108 mg\/dL/i);
  assert.match(evidence.map((item) => item.text).join(' '), /SpO2/i);
});
