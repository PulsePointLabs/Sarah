...
import { REVIEW_VIDEO_RENDER_VERSION } from '../../src/lib/reviewVideoVersion.js';

const REVIEW_RENDER_VERSION = REVIEW_VIDEO_RENDER_VERSION;
...
async function renderSegmentedSourceReviewVideo({
  ...
  const manifest = {
    ...
    telemetry_overlay: {
      enabled: Boolean(telemetryData?.rows?.length || Number(telemetryData?.avg) > 0),
      position: 'upper_right',
      composited_after_anatomical_zoom: true,
      source: telemetryData?.rows?.length ? 'session_hr_csv' : Number(telemetryData?.avg) > 0 ? 'session_hr_summary' : 'unavailable',
      row_count: telemetryData?.rows?.length || 0,
      average_bpm: Number.isFinite(Number(telemetryData?.avg)) ? Math.round(Number(telemetryData.avg)) : null,
      maximum_bpm: Number.isFinite(Number(telemetryData?.max)) ? Math.round(Number(telemetryData.max)) : null,
    },
