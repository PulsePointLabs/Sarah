...
export function buildGlobalProfileContext(userProfile) {
  ...
  if (latestComposition) {
    addLine(lines, "Latest measured weight", latestComposition.weight_kg != null ? `${kilogramsToPounds(latestComposition.weight_kg).toFixed(1)} pounds` : "");
    ...
    addLine(lines, "Latest lean body mass estimate", latestComposition.lean_body_mass_kg != null ? `${kilogramsToPounds(latestComposition.lean_body_mass_kg).toFixed(1)} pounds` : "");
    addLine(lines, "Latest body-composition measurement time", latestComposition.measured_at);
  } else {
    addLine(lines, "Profile weight", userProfile.weight_kg ? `${kilogramsToPounds(userProfile.weight_kg).toFixed(1)} pounds` : "");
  }
  lines.push("Weight-unit preference: use pounds (lb) as the primary unit in responses. Preserve kilograms only when explicitly discussing source/import units or when a metric requires them.");
