...
export default function HowlTimelineCard({ rows = [], error, onRetry, playheadS = 0, xDomain, onSeek, compact = false, offset = 0 }) {
  ...
  return <section aria-label="Howl settings timeline" className={`relative rounded-xl border border-border bg-background/60 ${compact?(selected?'flex min-h-0 flex-1 flex-col p-1.5':'shrink-0 p-1.5'):'p-3'}`}>
    <div className="flex shrink-0 flex-wrap items-center justify-between gap-2">
      <h3 className="text-[10px] font-semibold uppercase tracking-wider text-primary">{commandOnly?'Howl · saved controls':'Howl'}</h3>
    ...
    {selected&&<>
      <div className="mb-1 flex shrink-0 items-center gap-3 text-[9px] text-muted-foreground"><span>{selected.label} · {selected.unit}</span>{selected.lines.map(([key,label,color])=><span key={key} style={{color}}>{label}</span>)}</div>
