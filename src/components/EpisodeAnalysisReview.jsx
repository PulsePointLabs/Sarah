...
export default function EpisodeAnalysisReview({episode, review, onAnalyze, disabled, onSeek}) {
  ...
  const result = review?.result, job = review?.job;
  ...
  return <div className="mt-3 rounded-xl border border-primary/15 bg-background/30 p-3 text-xs">
    ...
    {result && <>
      ...
      <details className="mt-3"><summary className="cursor-pointer text-primary">Head-to-toe visual checklist · {result.source.role === 'feet' ? 'Feet / lower body only' : 'Main'} · {result.segments.length} segments</summary>
      ...
      <details className="mt-3"><summary className="cursor-pointer text-primary">Compared with earlier episodes ({result.synthesis.comparisons.length})</summary>
        ...
        {!result.synthesis.comparisons.length && <p className="mt-2 text-muted-foreground">No earlier episode comparison was available.</p>}
      </details>
