...
function buildCSV(session, timelineRows) {
  ...
  lines.push(`Release Completeness,${s.release_completeness || ""}`);
  lines.push(`Arousal Depth,${s.arousal_depth || ""}`);
  lines.push(`Response Stability,${s.erection_stability || ""}`);
  lines.push(`Stimulation Fit,${s.stimulation_fit || ""}`);
  lines.push(`Edge Control Quality,${s.control || ""}`);
  lines.push(`Sensory Immersion,${s.sensory_immersion || ""}`);
  lines.push(`Recovery Quality,${s.recovery_quality || ""}`);
  lines.push(`Discomfort Interruption Impact,${s.discomfort_interference || ""}`);
  lines.push(`Primary Limiting Factor,${s.primary_limiting_factor || s.no_climax_stop_reason || ""}`);
...
function buildTextReport(session, timelineRows) {
  ...
  if (s.release_completeness) lines.push(`  Release:       ${s.release_completeness}/10`);
  if (s.arousal_depth) lines.push(`  Arousal Depth: ${s.arousal_depth}/10`);
  if (s.erection_stability) lines.push(`  Stability:     ${s.erection_stability}/10`);
  if (s.stimulation_fit) lines.push(`  Stim Fit:      ${s.stimulation_fit}/10`);
  if (s.control) lines.push(`  Control:       ${s.control}/10`);
  if (s.sensory_immersion) lines.push(`  Immersion:     ${s.sensory_immersion}/10`);
  if (s.recovery_quality) lines.push(`  Recovery:      ${s.recovery_quality}/10`);
  if (s.discomfort_interference) lines.push(`  Interference:  ${s.discomfort_interference}/10`);
  if (s.primary_limiting_factor || s.no_climax_stop_reason) lines.push(`  Limiting:      ${s.primary_limiting_factor || s.no_climax_stop_reason}`);
  lines.push(`  Build Type:    ${s.build_type || "—"}`);
