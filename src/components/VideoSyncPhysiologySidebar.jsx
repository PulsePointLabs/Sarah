      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <XAxis
            ...
            tickCount={6}
          />
          <YAxis yAxisId="left" domain={["auto", "auto"]} tick={{ fontSize: 8, fill: "hsl(var(--muted-foreground))" }} />
          {rightAxis && (
            <YAxis
              yAxisId="right"
              orientation="right"
...
export default function VideoSyncPhysiologySidebar({
  ...
  const sdnn = positiveOrNull(current?.hrv_sdnn_ms);
  ...
  const priorReading = (readings) => readings
    .map((reading) => ({ ...reading, t: numberOrNull(reading?.time_offset_s ?? reading?.time_s ?? reading?.t) }))
    .filter((reading) => reading.t != null && reading.t <= playheadS)
    .sort((left, right) => right.t - left.t)[0] || null;
  const currentBp = useMemo(() => priorReading(bloodPressureReadings), [bloodPressureReadings, playheadS]);
  const currentSpo2 = useMemo(() => priorReading(pulseOxReadings), [pulseOxReadings, playheadS]);
  const bpSystolic = positiveOrNull(currentBp?.systolic_mm_hg ?? currentBp?.systolic);
  const bpDiastolic = positiveOrNull(currentBp?.diastolic_mm_hg ?? currentBp?.diastolic);
  const spo2 = positiveOrNull(currentSpo2?.spo2_percent ?? currentSpo2?.spo2 ?? currentSpo2?.oxygen_saturation);
  const bpAge = currentBp ? Math.max(0, playheadS - Number(currentBp.t)) : null;
  const spo2Age = currentSpo2 ? Math.max(0, playheadS - Number(currentSpo2.t)) : null;
    ...
    <section className={`${compact ? "flex h-full min-h-0 flex-col gap-2 overflow-hidden p-2" : "space-y-3 p-3"} rounded-2xl border border-primary/15 bg-gradient-to-b from-primary/[0.055] via-card to-card shadow-sm`}>
      ...
      <div className={`grid gap-2 ${compact ? "grid-cols-3" : "grid-cols-2"}`}>
        ...
        <MetricCard
          ...
          unit="mmHg"
          detail={currentBp ? `${formatTime(currentBp.t)} · ${Math.round(bpAge)}s old` : "No prior BP reading in this session"}
        ...
        {optionalChannels.spo2 !== false && pulseOxReadings.length > 0 && <MetricCard
          ...
          unit="%"
          detail={currentSpo2 ? `${formatTime(currentSpo2.t)} · ${Math.round(spo2Age)}s old` : "No prior SpO2 reading"}
          tone="text-cyan-500"
          compact={compact}
        />}
      ...
      {!compact && <div className="grid grid-cols-2 gap-2 text-[9px]">
