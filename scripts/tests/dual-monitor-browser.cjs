...
(async () => {
  ...
  try {
    ...
    const videoElement = page.locator('video:visible').first();
    const unzoomedBox = await videoElement.boundingBox();
    await page.keyboard.press('+');
    assert.match(await videoElement.getAttribute('style'), /scale\(1.25\)/);
    const beforePan = await videoElement.evaluate(v=>v.currentTime);
    await monitor.keyboard.press('ArrowRight');
    await monitor.keyboard.press('ArrowDown');
    assert.match(await videoElement.getAttribute('style'), /translate\(-5%, -5%\)/);
    assert.equal(await videoElement.evaluate(v=>v.currentTime),beforePan);
    await monitor.keyboard.press('-');
    assert.match(await videoElement.getAttribute('style'), /translate\(0%, 0%\) scale\(1\)/);
    assert.deepEqual(await videoElement.boundingBox(),unzoomedBox);
    ...
    assert.match(await videoElement.getAttribute('style'), /scale\(1\)/);
