...
(async () => {
  ...
  try {
    ...
    await assertFits();
    await page.locator('[data-telemetry-item="metric:SDNN"]').waitFor();
    await page.locator('[data-telemetry-item="metric:Respiration"]').click();
    await page.getByRole('button', { name: 'Hide card', exact: true }).click();
    assert.equal(await page.locator('[data-telemetry-item="metric:Respiration"]').count(), 0);
    assert.equal(await page.locator('[data-telemetry-item="metric:Chest Motion"]').count(), 1);
    await page.reload();
    await page.locator('[data-telemetry-item="metric:Chest Motion"]').waitFor();
    assert.equal(await page.locator('[data-telemetry-item="metric:Respiration"]').count(), 0, 'hidden state persists');
    await page.getByRole('button', { name: 'Telemetry controls' }).click();
    await page.getByRole('button', { name: 'Choose items' }).click();
    await page.getByRole('checkbox', { name: 'Show Respiration card', exact: true }).check();
    await page.getByRole('button', { name: 'Use this dashboard', exact: true }).click();
    await page.locator('[data-telemetry-item="metric:Respiration"]').waitFor();
    await page.locator('[data-telemetry-item="phase"]').click();
    const statsBefore = await page.locator('.telemetry-phase-stats').boundingBox();
    await page.getByRole('button', { name: 'Taller', exact: true }).click();
    await page.getByRole('button', { name: 'Taller', exact: true }).click();
    await page.getByRole('button', { name: 'Done', exact: true }).click();
    await page.waitForTimeout(200);
    const statsAfter = await page.locator('.telemetry-phase-stats').boundingBox();
    assert.ok(statsAfter.height > statsBefore.height, 'phase statistic cells expand vertically');
    const metric = page.locator('[data-telemetry-item="metric:Respiration"] .telemetry-metric-main');
    const anchoredBefore = await metric.boundingBox();
    const plotBefore = await page.locator('[data-telemetry-item="threshold"] .telemetry-trend-plot').boundingBox();
    const phaseBefore = await page.locator('.telemetry-phase-stats').boundingBox();
    await page.evaluate(() => window.emitTelemetryFixture('hr_telemetry', { source: 'direct_h10', currentHr: 124, baselineHr: 85, hrv: { rmssdMs: 42, sdnnMs: 51, sampleCount: 180, quality: 'high' }, multimodal: { respiration: { available: false, reason: 'collecting a longer sensor window with low periodicity and movement interference' } }, received_at: new Date().toISOString() }));
    await page.waitForTimeout(300);
    const anchoredAfter = await metric.boundingBox();
    const plotAfter = await page.locator('[data-telemetry-item="threshold"] .telemetry-trend-plot').boundingBox();
    assert.ok((await page.locator('[data-telemetry-item="metric:SDNN"]').innerText()).includes('51.0'));
    assert.equal(await page.locator('[aria-label="Session timer"]').evaluate(el => getComputedStyle(el).fontSize), await page.locator('[data-telemetry-header] h3 span').evaluate(el => getComputedStyle(el).fontSize));
    const phaseAfter = await page.locator('.telemetry-phase-stats').boundingBox();
    for (const key of ['x', 'y', 'width', 'height']) {
      assert.ok(Math.abs(anchoredAfter[key] - anchoredBefore[key]) < 1, `metric ${key} stays anchored`);
      assert.ok(Math.abs(plotAfter[key] - plotBefore[key]) < 1, `graph ${key} stays anchored`);
      assert.ok(Math.abs(phaseAfter[key] - phaseBefore[key]) < 1, `phase ${key} stays anchored`);
    }
