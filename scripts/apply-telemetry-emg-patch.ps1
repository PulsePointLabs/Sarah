...
if ($installed.name -ne 'sarah-standalone' -or $installed.version -notin @('0.1.266', '0.1.267', '0.1.268', '0.1.269')) {
    throw 'This patch requires the Sarah 0.1.266 through 0.1.269 Windows installation.'
}
...
$backup = Join-Path $InstallDirectory ('patch-backups\0.1.269-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
foreach ($entry in $manifest) {
...
Write-Host "Sarah 0.1.269 is ready. Previous files are backed up at $backup"
